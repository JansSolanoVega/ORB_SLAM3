...
def align(model, data):  # cuz monocular obslam doesnt get scale
...
if __name__ == "__main__":
    ...
    ground_data = np.loadtxt(
        "/home/zetans/VisualSLAM/kitti_dataset/poses/" + sequence_txt + ".txt"
    )
    ...
    # print(re_fpoints[0])
    # print(trans_error)
    ground_traj = plt.scatter(ground_points[0], ground_points[2], s=0.1)
    aligned_estimated_traj = plt.scatter(
        list(re_fpoints[0]), list(re_fpoints[2]), s=0.1, c="red"
    )
    # estimated_traj = plt.scatter(list(re_points[0]), list(re_points[2]), s=0.1, c="green")
    ...
    plt.legend(
        (ground_traj, aligned_estimated_traj),
        ("Ground traj", "Aligned estimated traj"),
        scatterpoints=1,
        loc="upper right",
        ncol=1,
        fontsize=8,
